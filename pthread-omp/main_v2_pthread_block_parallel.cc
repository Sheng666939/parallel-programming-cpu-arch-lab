...
void *pthreadNttWorker(void *arg) {
    ...
    for (int butterflySize = 1; butterflySize < ctx->transformLength; butterflySize <<= 1) {
        ...
        // block 级循环划分：第 threadId 个线程处理 threadId, threadId+threadCount, ...
        for (int blockIndex = threadId; blockIndex < blockCount; blockIndex += ctx->threadCount) {
            int blockStart = blockIndex * blockSize;
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Pthread thread count = " << threadCount << std::endl;
    int test_begin = 0;
    int test_end = 3;
