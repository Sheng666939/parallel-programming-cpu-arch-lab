...
static int crtStageRootsForwardMontgomery[CRT_MOD_COUNT][300000];
static int crtStageRootsInverseMontgomery[CRT_MOD_COUNT][300000];
...
static inline uint64_t computeBarrettReciprocal64(uint32_t modulus) {
    return static_cast<uint64_t>((static_cast<__uint128_t>(1) << 64) / modulus);
}
static inline int multiplyModCrtBarrett(
    int x,
    int y,
    int modulus,
    uint64_t barrettReciprocal
) {
    uint64_t value =
        static_cast<uint64_t>(static_cast<uint32_t>(x)) *
        static_cast<uint32_t>(y);

    uint64_t quotient = static_cast<uint64_t>(
        (static_cast<__uint128_t>(value) * barrettReciprocal) >> 64
    );

    uint64_t reduced = value - quotient * static_cast<uint64_t>(modulus);

    while (reduced >= static_cast<uint64_t>(modulus)) {
        reduced -= static_cast<uint64_t>(modulus);
    }
...
static inline long long subtractModNoDivision(long long left, long long right, int modulus) {
    // left and right are already reduced. Their difference lies in (-modulus, modulus),
    // so a single conditional add replaces a costly '%' operation.
...
void prepareCrtStageRootTableMontgomery(
    int transformLength,
    int modulusIndex,
    bool inverseTransform
) {
    ...
    const uint64_t barrettReciprocal =
        computeBarrettReciprocal64(modulusUnsigned);
    int *rootTable = inverseTransform
        ? crtStageRootsInverseMontgomery[modulusIndex]
        : crtStageRootsForwardMontgomery[modulusIndex];
    for (int butterflySize = 1;
         butterflySize < transformLength;
         butterflySize <<= 1) {
        int blockSize = butterflySize << 1;
        ...
        int currentRoot = 1;
        for (int offset = 0; offset < butterflySize; ++offset) {
            rootTable[butterflySize + offset] = static_cast<int>(
                static_cast<uint64_t>(currentRoot) *
                montgomeryR %
                modulusUnsigned
            );
            currentRoot = multiplyModCrtBarrett(
                currentRoot,
                rootStep,
                modulus,
                barrettReciprocal
...
void numberTheoreticTransformCrtModMontgomeryPrepared(
    int *data,
    int transformLength,
    int modulusIndex,
    bool inverseTransform
    ...
    const uint64_t barrettReciprocal =
        computeBarrettReciprocal64(modulusUnsigned);
    const int *rootTable = inverseTransform
        ? crtStageRootsInverseMontgomery[modulusIndex]
        : crtStageRootsForwardMontgomery[modulusIndex];
    for (int butterflySize = 1;
         butterflySize < transformLength;
         butterflySize <<= 1) {
        int blockSize = butterflySize << 1;
        const int *stageRootBase = rootTable + butterflySize;
        for (int blockStart = 0;
             blockStart < transformLength;
             blockStart += blockSize) {
            for (int offset = 0; offset < butterflySize; ++offset) {
                ...
                uint64_t product =
                    ...
                    static_cast<uint32_t>(stageRootBase[offset]);
                int lowerValue = static_cast<int>(
                    montgomeryReduce32(
                        product,
                        modulusUnsigned,
...
// This function is executed inside one existing OpenMP parallel region.
// All OpenMP worker threads call it. Each pair/group of threads works on one CRT modulus.
// Global OpenMP barriers are used as group synchronization points. Because all four CRT
// transforms have the same transformLength and the same stage structure, every thread
...
static inline long long garnerCombineToTargetModulusFast4(
    ...
    long long x1 = subtractModNoDivision(
        residue1,
        x0,
        CRT_MODS[1]
    );
    x1 = x1 * precomputed.inv01 % CRT_MODS[1];
    long long x2 = subtractModNoDivision(
        residue2,
        x0,
        CRT_MODS[2]
    );
    x2 = x2 * precomputed.inv02 % CRT_MODS[2];
    x2 = subtractModNoDivision(
        x2,
        x1,
        CRT_MODS[2]
    );
    x2 = x2 * precomputed.inv12 % CRT_MODS[2];
    long long x3 = subtractModNoDivision(
        residue3,
        x0,
        CRT_MODS[3]
    );
    x3 = x3 * precomputed.inv03 % CRT_MODS[3];
    x3 = subtractModNoDivision(
        x3,
        x1,
        CRT_MODS[3]
    );
...
void multiplyByNttLargeCRTModParallel(
    ...
    for (int modulusIndex = 0;
        ...
        int modulus = CRT_MODS[modulusIndex];
        uint64_t barrettReciprocal =
            computeBarrettReciprocal64(static_cast<uint32_t>(modulus));
        for (int index = 0; index < transformLength; ++index) {
        ...
        for (int index = 0; index < transformLength; ++index) {
            crtWorkA[modulusIndex][index] = multiplyModCrtBarrett(
                crtWorkA[modulusIndex][index],
                crtWorkB[modulusIndex][index],
                modulus,
                barrettReciprocal
    ...
    int resultLength = 2 * polynomialLength - 1;
    CrtGarnerPrecomputed precomputed =
        buildCrtGarnerPrecomputed(targetModulus);
    #pragma omp parallel for num_threads(threadCount) schedule(static)
    for (int index = 0; index < resultLength; ++index) {
...
#ifndef USE_GROUPED_CRT
#define USE_GROUPED_CRT 0
#endif
void multiplyByNttLargeCRT(
    ...
    // V10-6 default:
    // Keep the stable four-modulus outer-parallel CRT. The grouped path is still
    // available by compiling with -DUSE_GROUPED_CRT=1, but the default path avoids
    // the heavy global barriers of grouped NTT and instead precomputes twiddle
    // tables plus removes several '%' operations from fast Garner.
#if USE_GROUPED_CRT
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "32-bit Pthread whole-pipeline + large-modulus CRT V10-6 thread count = "<< threadCount << std::endl;
    int test_begin = 0;
    int test_end = 4;
