...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Pthread whole-pipeline thread count = "<< threadCount << std::endl;
    int test_begin = 0;
    int test_end = 4;
    for(int i = test_begin; i <= test_end; ++i){
        if (i <= 3) {
            ...
            auto Start = std::chrono::high_resolution_clock::now();
            multiplyByNttLargeOpenMP(aLarge, bLarge, abLarge, n_, p_, threadCount);
            auto End = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::ratio<1,1000>> elapsed =End - Start;
            ans += elapsed.count();
            fCheckLarge(abLarge, n_, i);
            std::cout << "large modulus OpenMP latency for n = "<< n_<< " p = "<< p_<< " : "<< ans<< " (ms) "<< std::endl;
        }
    }
    return 0;
}
