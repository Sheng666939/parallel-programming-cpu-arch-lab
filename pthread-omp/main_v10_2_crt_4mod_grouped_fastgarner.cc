...
static inline long long garnerCombineToTargetModulusFast4(
    ...
    long long x1 = positiveModLongLong(
        residue1 - x0,
        CRT_MODS[1]
    );
    x1 = x1 * precomputed.inv01 % CRT_MODS[1];

    long long x2 = positiveModLongLong(
        residue2 - x0,
        CRT_MODS[2]
    );
    x2 = x2 * precomputed.inv02 % CRT_MODS[2];
    x2 = positiveModLongLong(
        x2 - x1,
        CRT_MODS[2]
    );
    x2 = x2 * precomputed.inv12 % CRT_MODS[2];

    long long x3 = positiveModLongLong(
        residue3 - x0,
        CRT_MODS[3]
    );
    x3 = x3 * precomputed.inv03 % CRT_MODS[3];
    x3 = positiveModLongLong(
        x3 - x1,
        CRT_MODS[3]
    );
...
void multiplyByNttLargeCRT(
    ...
    // V10-2:
    // Use four CRT moduli. When THREAD_COUNT = 8, two OpenMP worker threads
    // cooperate on each modulus, so the CRT stage no longer leaves half of the
    // threads idle. For smaller thread counts, fall back to the safer V10-1
    // modulus-level parallel CRT.
