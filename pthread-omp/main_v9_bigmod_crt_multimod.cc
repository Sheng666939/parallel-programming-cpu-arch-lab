...
static int crtStageRoots[CRT_MOD_COUNT][300000];
...
int primitiveRootPrimeCrt(int modulus) {
    int phi = modulus - 1;
    int value = phi;
    ...
    for (int root = 2; root < modulus; ++root) {
        bool valid = true;

        for (int factor : factors) {
            if (quickPowerModCrtInt(root, phi / factor, modulus) == 1) {
                valid = false;
                break;
            }
        }
...
void numberTheoreticTransformCrtMod(
    int *data,
    int transformLength,
    int modulus,
    int primitiveRoot,
    ...
    for (int butterflySize = 1;
        ...
        for (int offset = 1; offset < butterflySize; ++offset) {
            stageRoots[offset] = multiplyModCrtInt(
                stageRoots[offset - 1],
                rootStep,
                modulus
            );
        ...
        for (int blockStart = 0;
            ...
            for (int offset = 0; offset < butterflySize; ++offset) {
                ...
                int lowerValue = multiplyModCrtInt(
                    data[lowerIndex],
                    stageRoots[offset],
                    modulus
                );
...
void multiplyByNttLargeCRT(
    ...
    int primitiveRoots[CRT_MOD_COUNT];

    for (int modulusIndex = 0; modulusIndex < CRT_MOD_COUNT; ++modulusIndex) {
        primitiveRoots[modulusIndex] = primitiveRootPrimeCrt(
            CRT_MODS[modulusIndex]
        );
    }

    // 多模数并行：每个线程负责一个或多个小模数下的 NTT。
    // 这正是 CRT 多线程优化的核心思想。
    #pragma omp parallel for num_threads(threadCount) schedule(static)
    for (int modulusIndex = 0;
        ...
        numberTheoreticTransformCrtMod(
            crtWorkA[modulusIndex],
            transformLength,
            modulus,
            primitiveRoots[modulusIndex],
            false,
            crtStageRoots[modulusIndex]
        );
