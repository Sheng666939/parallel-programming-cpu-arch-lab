...
\begin{titlepage}
    \begin{center}
    \includegraphics[width=0.62\textwidth]{NKU.png}\\[0.9cm]
    \textsc{\Huge \kaishu{\textbf{南\ \ \ \ \ 开\ \ \ \ \ 大\ \ \ \ \ 学}}}\\[0.65cm]
    \textsc{\huge \kaishu{\textbf{计\ \ 算\ \ 机\ \ 学\ \ 院}}}\\[0.35cm]
    \textsc{\Large \textbf{并行程序设计 Pthread \& OpenMP 编程实验报告}}\\[0.75cm]
    \HRule \\[0.65cm]
    {\LARGE \bfseries 基于 Pthread、OpenMP 与 CRT 的\\[0.3cm]NTT 多项式乘法并行优化}\\[0.55cm]
    \HRule \\[1.65cm]

    \begin{tabular}{rl}
    \Large \kaishu{姓\qquad 名：} & \Large \kaishu{罗晟皓}\\[0.35cm]
    \Large \kaishu{学\qquad 号：} & \Large \kaishu{2411566}\\[0.35cm]
    \Large \kaishu{专\qquad 业：} & \Large \kaishu{计算机科学与技术}\\[0.35cm]
    \Large \kaishu{实验名称：} & \Large \kaishu{Pthread \& OpenMP 多线程编程}\\[0.35cm]
    \Large \kaishu{选\qquad 题：} & \Large \kaishu{NTT 多项式乘法优化}\\[0.35cm]
    \Large \kaishu{运行平台：} & \Large \kaishu{OpenEuler / Kunpeng-920}
    \end{tabular}
...
%================================================================
\section{实验环境、数据集与测试方法}
%================================================================

\subsection{实验平台与程序框架}

本实验在课程提供的统一测试环境中完成，程序运行平台为 OpenEuler / Kunpeng-920。实验代码使用
C++ 实现，其中 OpenMP 版本需要开启 OpenMP 支持，Pthread 版本需要链接 pthread 库。为了便于在
不同线程数下重复实验，代码中通过 \texttt{THREAD\_COUNT} 宏或编译脚本传入线程数量，正式测试中
主要使用 $1/2/4/8$ 线程。最终提交版本在前三个 32 位输入上调用 Pthread whole-pipeline 主线，
在大模数输入上调用 4-mod grouped CRT 版本。

程序沿用实验框架提供的 \texttt{fRead}、\texttt{fCheck}、\texttt{fReadLarge} 和
\texttt{fCheckLarge} 接口。计时区间只覆盖多项式乘法函数本身：输入读取发生在计时之前，
正确性检查发生在计时之后。因此表中时间主要反映 NTT/CRT 计算过程，而不包含文件读写和结果检查开销。

\subsection{测试数据}

测试数据共包括 5 个输入文件。第 0 个输入用于小规模正确性调试；第 1--3 个输入使用 32 位
NTT-friendly 模数；第 4 个输入使用大模数，要求额外实现 64 位/CRT 方案。本文正式性能分析主要关注
$n=131072$ 的大规模输入，同时保留小规模输入用于验证程序逻辑。

\begin{table}[H]
  \centering
  \begin{tabular}{cccl}
  \toprule
  \textbf{输入编号} & \textbf{多项式长度 $n$} & \textbf{模数 $p$} & \textbf{用途}\\
  \midrule
  input 0 & 4      & 7340033              & 小规模调试与正确性验证\\
  input 1 & 131072 & 7340033              & 32 位 NTT 性能测试\\
  input 2 & 131072 & 104857601            & 32 位 NTT 性能测试\\
  input 3 & 131072 & 469762049            & 32 位 NTT 性能测试\\
  input 4 & 131072 & 1337006139375617     & 大模数 NTT / CRT 测试\\
  \bottomrule
  \end{tabular}
  \caption{实验输入数据规模与用途}
  \label{tab:dataset}
...
对于 $n=131072$ 的输入，卷积结果长度为 $2n-1=262143$，NTT 变换长度取向上补齐的二次幂
$L=262144=2^{18}$。这意味着一次多项式乘法需要两次正向 NTT、一次点值乘法和一次逆 NTT，
单次 NTT 内部共有 $18$ 个 stage。每个 stage 内总 butterfly 数均为 $L/2=131072$，
但 block 数随 stage 变化而从 $L/2$ 逐层下降到 $1$，这正是后文 Pthread 任务划分分析的关键。

\subsection{计时与数据解释}

代码中使用 \texttt{std::chrono::high\_resolution\_clock} 记录一次多项式乘法调用的耗时。
早期输出中变量名使用了 \texttt{average latency}，但部分版本每次只运行一次；在最终报告中，
只有明确多次复跑的数据才称为平均值。例如 V10-2 大模数 CRT 版本进行了 3 次独立复跑，报告中采用
三次平均 $33.6254$\,ms 作为正式数据，同时记录最佳单次 $32.9417$\,ms。

为了避免把偶然波动误认为算法收益，本文在讨论时优先比较同一批输入、同一线程数下的版本差异。
对于 OpenMP 与 Pthread 的线程扩展性，则统一以各自 1 线程结果作为基准计算加速比和并行效率。

\subsection{版本命名约定}

由于本实验经历了多轮实现和优化，本文使用版本号 V0--V10-2 来描述代码演进。版本命名仅用于报告组织，
其含义如表~\ref{tab:version-overview} 所示。
...
\begin{table}[H]
  ...
  \textbf{版本} & \textbf{含义}\\
  \midrule
  V0 & 32 位标量迭代 NTT baseline，使用 Montgomery reduction，不使用多线程\\
  V1 & OpenMP 多线程 NTT，stage 内 block 并行\\
  V2 & Pthread block-parallel，按 block 划分每层任务\\
  V3 & Pthread flat-butterfly，将每层 butterfly 展平成线性任务\\
  V4 & Pthread contiguous flat-butterfly，尝试连续区间划分\\
  V5 & Pthread whole-pipeline thread pool，整个多项式乘法只创建一次线程组\\
  V6 & Pthread whole-pipeline + cyclic-flat，作为过细任务划分的反例\\
  V7 & 直接大模数标量 NTT，使用 \texttt{\_\_int128} 模乘\\
  V8 & 直接大模数 OpenMP NTT\\
  V9 & 5-mod CRT 初版\\
  V10-1 & 4 模数 + Montgomery CRT\\
  V10-2 & 4-mod grouped CRT + Montgomery NTT + Fast Garner，最终大模数主推版本\\
  \bottomrule
  \end{tabular}
  \caption{本文版本命名约定}
  \label{tab:version-overview}
\end{table}
...
\subsection{并行正确性的基本不变量}

NTT 并行化时需要保证两个不变量。第一，同一 stage 内每个 butterfly 对应一对唯一的数据位置
$(k+j, k+j+b)$，不同 butterfly 之间不重叠，因此可以并行写入。第二，第 $s+1$ 层 stage 的输入依赖
第 $s$ 层全部 butterfly 的输出，因此 stage 之间必须进行全局同步。OpenMP 版本依赖 \texttt{for}
和 \texttt{single} 的隐式 barrier；Pthread 版本显式调用 \texttt{pthread\_barrier\_wait}。
只要每个 stage 满足“先预计算 roots、再并行 butterfly、最后 barrier”的顺序，数据一致性即可保证。

\subsection{任务规模变化对并行化的影响}

对于变换长度 $L=2^{18}$，第 $s$ 层的 \texttt{butterflySize} 为 $2^s$，block 数为
\[
  \mathrm{blockCount}=\frac{L}{2\cdot \mathrm{butterflySize}}.
\]
若按 block 划分任务，前期 stage 的 block 数很多，线程容易均衡；但后期 stage 的 block 数会变成
$4,2,1$，此时 8 线程中只有少数线程真正工作，其他线程只能等待。这解释了 V2 block-parallel 在后期
stage 的负载不均。相反，若按 butterfly 展平成线性任务，每层总任务数恒为
\[
  \mathrm{totalButterflies}=L/2,
\]
负载均衡更好，但需要从 butterfly id 反推出 block 与 offset，可能引入整数除法和局部性损失。
因此 NTT 的并行设计需要在负载均衡、索引计算开销和 cache 局部性之间折中。
...
\subsection{基线实现中的关键优化}

虽然 V0 是单线程版本，但它并非朴素 $O(n^2)$ 卷积，而是已经完成了 NTT 层面的算法优化。
其主要优化包括：

\begin{enumerate}
  \item \textbf{迭代原地 NTT}：避免递归函数调用和额外数组分配，所有 butterfly 都在原数组中完成；
  \item \textbf{stageRoots 预计算}：同一 stage 中所有 block 使用相同的单位根序列，因此每层只计算一次
        \texttt{stageRoots}，避免在每个 block 内重复递推；
  \item \textbf{Montgomery 表示}：把单位根提前转成 Montgomery 形式，使 butterfly 中的乘法可以使用
        \texttt{montgomeryReduce32} 完成；
  \item \textbf{静态工作数组}：使用静态数组避免反复在栈上申请大数组，降低栈溢出风险和内存分配开销。
\end{enumerate}

这些优化保证了 V0 是一个相对公平的高质量串行基线。后续多线程版本的加速比并不是相对朴素卷积计算，
而是相对已经优化过的 $O(n\log n)$ NTT 实现，因此约 $2\times$ 的 8 线程加速是合理的。
...
\subsection{变量共享与同步关系}

OpenMP 版本中，输入数组 \texttt{data}、模数 \texttt{modulus}、单位根表
\texttt{stageRootsMontgomery} 为共享变量；循环变量、临时变量 \texttt{upperValue}、
\texttt{lowerValue}、\texttt{addedValue} 等为线程私有变量。每个线程只写自己负责的 block，
同一个 stage 内不会写到其他线程负责的数据位置，因此不需要 \texttt{critical} 或 \texttt{atomic}。
如果在 butterfly 中使用临界区，反而会将核心计算串行化，严重降低性能。

\texttt{single} 的隐式 barrier 很重要：线程 0 或任意一个 OpenMP 线程完成 roots 预计算后，
其他线程才能读取 \texttt{stageRootsMontgomery}。同理，\texttt{omp for} 末尾的隐式 barrier
保证当前 stage 完成后再进入下一层。该同步结构恰好对应 NTT 的算法依赖，因此 OpenMP 代码较为简洁。

\subsection{静态调度的选择}

本文 OpenMP 版本主要使用 \texttt{schedule(static)}。原因是 NTT 中每个 block 的工作量完全相同，
都包含 \texttt{butterflySize} 个 butterfly，动态调度不会改善负载均衡，反而会引入调度开销。
静态调度还能让线程获得连续 block 区间，较好地保持空间局部性。
...
\subsection{V5 的同步阶段划分}

V5 whole-pipeline 版本并不只是把某一个 NTT 循环并行化，而是把完整多项式乘法拆成多个阶段，
所有线程在同一个线程函数中依次完成这些阶段。阶段之间使用同一个 \texttt{pthread\_barrier\_t}
同步。其执行顺序可以概括为：

\begin{enumerate}
  \item 并行清零并复制输入；
  \item 线程 0 对第一个多项式做 bit-reversal，所有线程等待；
  \item 所有线程参与第一个多项式正向 NTT；
  \item 线程 0 对第二个多项式做 bit-reversal，所有线程等待；
  \item 所有线程参与第二个多项式正向 NTT；
  \item 并行点值乘法；
  \item 线程 0 对乘积数组做 bit-reversal，所有线程等待；
  \item 所有线程参与逆 NTT 和最终缩放；
  \item 并行拷贝真实长度的结果。
\end{enumerate}

这种设计的核心收益在于：三次 NTT、点值乘法和结果拷贝都复用同一批线程，不再为每一次 NTT
单独创建和销毁线程。对于 $L=2^{18}$ 这种规模，线程管理开销已经足以影响整体时间，因此 V5
相对 V2--V4 的提升非常明显。

\subsection{Pthread 实现中的工程风险}

Pthread 的灵活性也带来了更多工程风险。首先，barrier 数量和调用顺序必须在所有线程中完全一致，
否则会发生死锁。其次，线程参数结构体必须在所有线程结束前保持有效，不能使用临时局部变量地址。
再次，\texttt{stageRoots} 由线程 0 写入、所有线程读取，必须保证写入后存在 barrier。OpenMP 的
这些细节由运行时隐式处理，而 Pthread 必须由程序员显式保证，这也是 Pthread 代码量明显更大的原因。
...
\subsection{CRT 模数数量的正确性约束}

CRT 版本需要保证小模数乘积足以覆盖卷积系数的整数范围。对于长度 $n=131072$、目标模数
$P=1337006139375617$，单个卷积系数在取模前的理论上界可估计为
\[
  n(P-1)^2 \approx 2.3430\times 10^{35}.
\]
V10-2 使用的四个小模数乘积为
\[
\begin{aligned}
M &= 469762049 \times 754974721 \times 880803841 \times 998244353\\
  &= 311836107287235162503765106377097217 \\
  &\approx 3.1184\times 10^{35}.
\end{aligned}
\]
因此 $M > n(P-1)^2$，能够从四个小模数下的结果唯一恢复整数卷积系数，再对目标模数 $P$ 取模。
这也是最终版本能够从 5 个小模数减少到 4 个小模数而仍保持正确性的关键。
...
\subsection{最终版本在全部输入上的稳定性}

最终代码在前三个 32 位大规模输入上仍调用 Pthread V5 whole-pipeline 主线，在第四个大模数输入上调用
V10-2 CRT 主线。最近三次完整运行的前三个 32 位输入也保持稳定，说明加入 CRT 代码后没有破坏原有
Pthread 主线。三次运行中前三个大规模输入的平均值如表~\ref{tab:final-three-runs-32bit} 所示。

\begin{table}[H]
  \centering
  \begin{tabular}{lcccc}
  \toprule
  \textbf{模数} & \textbf{Run 1} & \textbf{Run 2} & \textbf{Run 3} & \textbf{平均}\\
  \midrule
  $7340033$   & 23.9839 & 23.8436 & 23.7730 & 23.8668\\
  $104857601$ & 24.1993 & 24.5114 & 24.0130 & 24.2412\\
  $469762049$ & 24.2725 & 24.4918 & 24.6455 & 24.4699\\
  \bottomrule
  \end{tabular}
  \caption{最终代码中前三个 32 位输入的三次复跑结果（单位：ms）}
  \label{tab:final-three-runs-32bit}
\end{table}

这组数据与前文 Pthread V5 的 8 线程结果基本一致，说明最终程序集成大模数 CRT 后，32 位 NTT 主线仍然稳定。
大模数输入三次复跑平均为 $33.6254$\,ms，最佳单次为 $32.9417$\,ms，成为本文最终最优进阶结果。
...
\subsection{从“能并行”到“并行得合理”}

本实验的实现过程表明，NTT 的多线程优化并不是简单地把某个外层循环加上线程即可。最初的
block-parallel 思路虽然最自然，也最接近 OpenMP 的 \texttt{for} 写法，但由于 NTT 后期 stage
的 block 数迅速减少，按 block 分配会造成明显的线程闲置。flat-butterfly 版本从任务建模层面
重新理解了 NTT：每一层实际拥有固定数量的 butterfly，而不是固定数量的 block。因此 V3 相对 V2
取得了直接收益。

但是，flat-butterfly 的进一步实验也说明“更均衡”并不总是“更快”。当每个 butterfly 都需要额外计算
\texttt{blockIndex}、\texttt{offset}、\texttt{upperIndex} 和 \texttt{lowerIndex} 时，索引计算成本
会显著增加；cyclic 分配还会让同一线程在内存中跳跃访问，破坏 cache 局部性。V6 的退化正是这一点的
直接证据。因此最终 Pthread 主线选择 V5 whole-pipeline，而不是看似更均衡的 cyclic-flat。

\subsection{线程管理开销的影响}

Pthread 与 OpenMP 的对比中，一个重要现象是：如果 Pthread 每次 NTT 都单独创建和销毁线程，
即使 stage 内计算本身并行，整体性能也很难接近 OpenMP。原因是一次多项式乘法包含三次 NTT：
两个输入多项式的正向 NTT 和乘积数组的逆 NTT。如果每次 NTT 都重新创建线程，则线程管理成本会被
重复放大。V5 将整个多项式乘法流程整合到同一个线程函数中，使线程在清零、拷贝、正变换、点值乘法、
逆变换和结果回拷之间持续复用。这一策略直接把 Pthread 主线从 $35$\,ms 量级推进到 $25$--$26$\,ms
量级，是 Pthread 路线中最关键的工程优化。

相比之下，OpenMP 的优势在于运行时自动完成了类似线程池的复用。只要程序员把
\texttt{parallel} 区域放在合适的位置，OpenMP 就能用较少代码获得稳定性能。因此最终报告中，
OpenMP 被作为“简洁高效”的对照，Pthread 则体现了“手写线程控制”的优化过程。

\subsection{CRT 优化路线的取舍}

大模数部分经历了从直接 \texttt{\_\_int128} 到 CRT 的转变。V7 和 V8 说明，多线程可以显著降低
直接大模数 NTT 的时间，但 \texttt{\_\_int128} 模乘仍然是高常数瓶颈。V9 初版 CRT 虽然成功把
大模数运算拆成小模数 NTT，但因为使用 5 个小模数，总 NTT 次数变多，并且外层模数级并行与 8 线程
不完全匹配，所以性能没有超过 V8。

V10-2 的最终设计综合考虑了正确性和线程利用率：四个小模数的乘积仍然大于卷积系数上界，因此可保证
CRT 重构正确；同时 4 个模数可以自然匹配 8 个线程，每个模数由 2 个线程协同完成 NTT。这个设计既减少了
模数数量，又避免了纯模数级并行的线程闲置。Fast Garner 进一步减少了合并阶段的重复模逆计算，使得
CRT 路线最终超过直接大模数 OpenMP 版本。

\subsection{最终提交版本的选择}

本文最终主推版本并不是单一函数，而是针对不同输入选择不同路线：前三个 32 位模数输入使用
Pthread whole-pipeline 主线，大模数输入使用 V10-2 grouped CRT 主线。这样做的原因是前三个模数
本身已经适合 32 位 Montgomery NTT，直接使用 Pthread V5 即可获得稳定性能；而第四个输入的模数过大，
直接做 \texttt{\_\_int128} NTT 会产生较高常数，适合切换到 CRT 路线。

最终版本的主要性能结论如表~\ref{tab:final-conclusion-table} 所示。表中列出的不是单次偶然结果，
而是报告中用于支撑结论的关键实验数据。
...
\begin{table}[H]
  ...
  \toprule
  \textbf{场景} & \textbf{最终采用策略} & \textbf{代表时间}\\
  \midrule
  32 位 OpenMP 对比 & V1 OpenMP 8 线程，block 级静态调度 & 21.7870 ms\\
  32 位 Pthread 主线 & V5 whole-pipeline thread pool，8 线程 & 23.2387 ms\\
  大模数直接实现 & V8 \texttt{\_\_int128} OpenMP，8 线程 & 45.704 ms\\
  大模数最终实现 & V10-2 4-mod grouped CRT，8 线程三次平均 & 33.6254 ms\\
  \bottomrule
  \end{tabular}
  \caption{最终提交版本的关键性能结论}
  \label{tab:final-conclusion-table}
\end{table}

\subsection{未继续采用的方向}

实验过程中也尝试或讨论过若干方向，但最终没有作为主线采用。第一，V6 的 cyclic-flat 虽然改善了理论
负载均衡，但实测显著退化，因此只作为反例分析。第二，预计算更多 roots 的版本曾被尝试，但在当前规模
下没有稳定超过 V10-2，因此未作为最终主推。第三，四分 NTT 属于 NTT 选题介绍中的进阶方向，但本实验
已完成 CRT 进阶优化，时间主要投入到大模数 CRT 的正确性和性能验证上，因此未实现四分 NTT。第四，
CRT 线程数扩展性没有像 32 位 NTT 那样完整测试 $1/2/4/8$ 线程；本文将线程扩展性分析放在基础的
OpenMP/Pthread 部分，CRT 部分重点比较不同大模数实现路线。
