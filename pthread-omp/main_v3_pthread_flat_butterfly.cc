...
void *pthreadNttWorker(void *arg) {
    ...
    for (int butterflySize = 1; butterflySize < ctx->transformLength; butterflySize <<= 1) {
        ...
        // flat-butterfly 任务划分：每一层都有 transformLength / 2 个 butterfly。
        // 这样即使后期 blockCount 很小，也能让所有线程分到计算任务。
        for (int butterflyId = threadId;
            butterflyId < totalButterflies;
            butterflyId += ctx->threadCount) {

            int blockIndex = butterflyId / butterflySize;
            int offset = butterflyId - blockIndex * butterflySize;
            int blockStart = blockIndex * blockSize;

            int upperValue = ctx->data[blockStart + offset];

            uint64_t product =
                static_cast<uint64_t>(
                    static_cast<uint32_t>(ctx->data[blockStart + offset + butterflySize])
                ) *
            ...
            ctx->data[blockStart + offset] = addedValue;
            ctx->data[blockStart + offset + butterflySize] = subtractedValue;
        }
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Pthread flat-butterfly thread count = " << threadCount << std::endl;
    int test_begin = 0;
    int test_end = 3;
    for(int i = test_begin; i <= test_end; ++i){
        ...
        multiplyByNttPthread(a, b, ab, n_, p_, threadCount);
