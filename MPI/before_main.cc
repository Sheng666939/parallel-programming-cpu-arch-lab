...
int main(int argc, char *argv[]) {
    ...
    std::cout << "32-bit Pthread whole-pipeline + large-modulus CRT thread count = "
              << threadCount
              << std::endl;
    ...
    for (int i = test_begin; i <= test_end; ++i) {
        if (i <= 3) {
            ...
            // 前三个 32-bit NTT-friendly 模数使用 Pthread whole-pipeline 版本
            ...
            std::cout << "average latency for n = "
                      << n_
                      << " p = "
                      << p_
                      << " : "
            ...
            // 第四个大模数输入使用 4-mod CRT + Montgomery NTT + Fast Garner
            ...
            std::cout << "large modulus CRT latency for n = "
                      << n_
                      << " p = "
                      << p_
                      << " : "
